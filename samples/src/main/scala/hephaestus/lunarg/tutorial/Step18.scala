...
object Step18 extends Utils {
  ...
  def main(args: Array[String]): Unit = {
    ...
    vk.bindImageMemory(device, textureImage, textureMemory, 0L)
    ...
    val textureDataPtr = vk.mapMemory(device,
                                      ...
                                      0)
    vk.loadMemory(textureDataPtr,
                  Cube.textureData(textureWidth, textureHeight, 0))
    ...
    val descriptorSets = vk.allocateDescriptorSets(
      device,
      new Vulkan.DescriptorSetAllocateInfo(descriptorPool = descriptorPool,
                                           setLayouts =
    ...
    val writeDescriptorSets = Array(
      new Vulkan.WriteDescriptorSet(
        ...
        bufferInfo = Array(
          new Vulkan.DescriptorBufferInfo(buffer = buffer,
                                          offset = 0L,
    ...
    (0 until 1000).foreach { i =>
      ...
      val textureDataPerFrame =
        Cube.textureData(textureWidth, textureHeight, i)
  ...
  def setImageLayout(oldLayout: Vulkan.ImageLayout,
                     newLayout: Vulkan.ImageLayout,
                     image: Vulkan.Image,
                     aspectMask: Vulkan.ImageAspectFlag,
                     srcStages: Vulkan.PipelineStageFlag,
                     dstStages: Vulkan.PipelineStageFlag,
                     commandBuffer: Vulkan.CommandBuffer) {

    val srcAccessMask = oldLayout match {
      case Vulkan.IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL =>
        Vulkan.ACCESS_COLOR_ATTACHMENT_WRITE_BIT
      case Vulkan.IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL =>
    ...
    val dstAccessMask = newLayout match {
      case Vulkan.IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL =>
        Vulkan.ACCESS_TRANSFER_WRITE_BIT
      case Vulkan.IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL =>
        Vulkan.ACCESS_TRANSFER_READ_BIT
      case Vulkan.IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL =>
        Vulkan.ACCESS_SHADER_READ_BIT
      case Vulkan.IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL =>
        Vulkan.ACCESS_COLOR_ATTACHMENT_WRITE_BIT
      case Vulkan.IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL =>
    ...
    val barrier = new Vulkan.ImageMemoryBarrier(
      ...
      image = image,
      subresourceRange = new Vulkan.ImageSubresourceRange(aspectMask =
                                                            aspectMask,
                                                          baseMipLevel = 0,
                                                          levelCount = 1,
                                                          baseArrayLayer = 0,
