...
If I want to draw 2 things with the same pipeline, I need the same layout
I need the same descriptor set (?)
...
object Step01 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val device = vk.createDevice(physicalDevice, new Vulkan.DeviceCreateInfo(
    ...
    val commandPool = vk.createCommandPool(device, new Vulkan.CommandPoolCreateInfo(
      ...
      queueFamilyIndex = qi))
    val primaryCommandBuffer = vk.allocateCommandBuffers(device, new Vulkan.CommandBufferAllocateInfo(
      commandPool = commandPool,
      level = Vulkan.COMMAND_BUFFER_LEVEL_PRIMARY,
      commandBufferCount = 1))
    val secondaryCommandBuffer = vk.allocateCommandBuffers(device, new Vulkan.CommandBufferAllocateInfo(
      commandPool = commandPool,
    ...
    val swapchain = vk.createSwapchain(device, new Vulkan.SwapchainCreateInfo(
      flags = 0,
      surface = surface,
    ...
    val imageViews = swapchainImages.map { i =>
      vk.createImageView(device, new Vulkan.ImageViewCreateInfo(
        flags = 0,
        image = i,
    ...
    val imageTiling = 
      if(Vulkan.FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT & formatProperties.linearTilingFeatures) Vulkan.IMAGE_TILING_LINEAR
    ...
    val depthImage = vk.createImage(device, new Vulkan.ImageCreateInfo(
      flags = 0,
    ...
    val depthImageMemory = vk.allocateMemory(device, new Vulkan.MemoryAllocateInfo(
    ...
    val depthImageView = vk.createImageView(device, new Vulkan.ImageViewCreateInfo(
      flags = 0,
      image = depthImage,
      viewType = Vulkan.IMAGE_VIEW_TYPE_2D,
      format = Vulkan.FORMAT_D16_UNORM,
    ...
    val uniformBuffer = vk.createBuffer(device, new Vulkan.BufferCreateInfo(
    ...
    val cubeDescriptorSetLayout = vk.createDescriptorSetLayout(device, new Vulkan.DescriptorSetLayoutCreateInfo(
      flags = 0,
      bindings = Array(
        new Vulkan.DescriptorSetLayoutBinding(
          binding = 0,
    ...
    val cubePipelineLayout = vk.createPipelineLayout(device, new Vulkan.PipelineLayoutCreateInfo(
      flags = 0,
      setLayouts = Array(cubeDescriptorSetLayout),
    ...
    val descriptorSets = vk.allocateDescriptorSets(device, new Vulkan.DescriptorSetAllocateInfo(
      ...
      setLayouts = Array(descriptorSetLayout, cubeDescriptorSetLayout)))
    ...
    vk.cmdBindDescriptorSets(secondaryCommandBuffer, Vulkan.PIPELINE_BIND_POINT_GRAPHICS, cubePipelineLayout,
